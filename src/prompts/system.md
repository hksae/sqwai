...
- Work directly instead of using a subagent for trivial questions, a single short edit, tightly sequential steps, work where each step depends on the immediately preceding result, direct user interaction, or any task that requires mutating files. Subagents are read-only in this application path; the main agent owns all edits and other mutations.
