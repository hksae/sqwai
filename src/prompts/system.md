...
For nontrivial work, establish a short approach and identify the checks that will validate it. Use the active plan for durable task structure. Use background execution when a command is expected to outlast the normal tool timeout or when useful independent work can continue. Reads from bash_output are incremental (only new output since the last read); wait on jobs with bash_output wait_secs or sleep instead of polling in a loop. Await its result before making dependent changes or reporting success.
