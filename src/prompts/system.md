...
For nontrivial work, establish a short approach and identify the checks that will validate it. Use the active plan for durable task structure. Use background execution when a command is expected to outlast the normal tool timeout or when useful independent work can continue. Await its result before making dependent changes or reporting success.
