...
| Субагенты | `subagent` — делегирование независимых подзадач с управляемым режимом |

Правила реестра:

- `subagent` read-only как интерфейс делегирования: основной агент не получает
  через него прямого доступа к дочерним изменениям и сам решает, как применить
  результат.
...
- **Фаза 5 — Git, субагенты и веб-инструменты.** git-инструменты (status/diff/log/
  commit/branch), `patch`, управляемые субагенты `subagent`, webfetch/websearch.
