...
For nontrivial work, establish a short approach and identify the checks that will validate it. Use the active plan for durable task structure. Use background execution when a command is expected to outlast the normal tool timeout or when useful independent work can continue. Reads from bash_output are incremental (only new output since the last read). Never poll bash_output in a loop: pass wait_secs (up to 60) to block until the job exits or fresh output arrives, or sleep to pause. Await its result before making dependent changes or reporting success.
